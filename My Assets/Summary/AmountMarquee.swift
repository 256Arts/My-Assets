        ...
        VStack {
            ...
            if showAsCombinedValue {
                VStack {
                    ...
                    Text("\(String(localized: period.displayName)) Net")
                        .font(.caption)
                ...
                HStack {
                    VStack {
                        Text(currencyDeltaFormatter.string(from: NSNumber(value: monthlyIncome * period.months))!)
                            ...
                            .accessibilityLabel("\(String(localized: period.displayName)) Income")
                        Text("\(String(localized: period.displayName)) Income")
                            .font(.caption)
                            .foregroundStyle(tertiaryLabel)
                    ...
                    VStack {
                        Text(currencyDeltaFormatter.string(from: NSNumber(value: -monthlyExpenses * period.months))!)
                            .font(Font.system(size: 22, weight: .medium, design: .rounded))
                            .foregroundStyle(.red)
                            .accessibilityLabel("\(String(localized: period.displayName)) Expenses")
                        Text("\(String(localized: period.displayName)) Expenses")
                            .font(.caption)
                            .foregroundStyle(tertiaryLabel)
                            .accessibilityHidden(true)
                    }
